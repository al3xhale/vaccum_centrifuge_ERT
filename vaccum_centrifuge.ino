/*  Arduino code for Vaccum Centrifuge project - Controls the two valves and motor speed.
...
// MOTOR EXPERIMENTAL CONSTANT (set experimentaly matching encoder rpm and reference rpm)

float K_motor = ((1/500)*255);

//TRAJECTORY DATA (matlab output)

int k_valve2 = 50; //discrete time to open second valve (calculated by matlab)

// the variable where the omega(t) trajectory is stored (calculated by matlab)
...
void loop() {
  ...
  if(t2==0){
   ...
   analogWrite(PWM,(int)(rpm_cmd*K_motor)); // send pwm command to motor
